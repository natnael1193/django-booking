...
def get_or_create(request):
    if request.method == "GET":
        room = Room.objects.filter(is_deleted=False).all()
...
def get_or_update_or_delete(request, pk):
    try:
        room = Room.objects.filter(is_deleted=False).get(pk=pk)
    except:
    ...
    if request.method == 'DELETE':
        serializer = RoomSerializer(room, data={"is_deleted": True}, partial=True)
        if serializer.is_valid():
            serializer.save()
